...
class APSchedulerService:
    ...
    def listener(self, event):
        ...
        if event.exception:
            ...
            logger.info("✅ SchedulerService выполнил задачу успешно.")
    ...
    async def run_price_update(self):
        ...
        try:
            ...
            for user_id, product_ids in users_products.items():
                ...
                for product_id in product_ids:
                    try:
                        ...
                        logger.info(
                            f"Для обновления цены в Scheduler передан result: {result}"
                        )
                ...
                if updated_products:
                    logger.info(
                        f"⚠️  Список товаров для отправки пользователю {updated_products}"
                    )
            ...
            for user_id, updated_products in notification_to_send.items():
                await self.notification_service.notify_multiple_products(
                    user_id, updated_products
