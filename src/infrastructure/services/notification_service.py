...
class NotificationService:
    ...
    async def notify_multiple_products(self, chat_id: str, updated_products: list):
        ...
        try:
            logger.info(
                f"📨 NotificationService: Отправка уведомления пользователю {chat_id}"
            )

            # Формируем текст сообщения с информацией о нескольких товарах
            products_count = len(updated_products)
            product_word = get_product_word_form(
                products_count
            ...
            for i, product in enumerate(updated_products):
                ...
                prev_price_without_card = product[
                ...
                button = InlineKeyboardButton(
                    text=f'{product["name"][:50]}', url=product["link"]
                )
                inline_buttons.append(
                    [button]
                )  # Добавляем кнопку в список как отдельный элемент
            ...
            logger.info(
                f"✅ Сообщение ID {message.message_id} отправлено пользователю {chat_id}"
            )
