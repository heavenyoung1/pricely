...
class UserRepositoryImpl(UserRepository):
    ...
    def save(self, user) -> None:
        ...
        try:
            ...
            existing_user = (
                self.session.query(ORMUser).filter(ORMUser.id == user.id).first()
            )

            if existing_user:
                # Если пользователь существует, используем merge для обновления
                logger.info(
                    f"Пользователь {user.id} уже существует. Выполняем обновление."
                )
                self.session.merge(UserMapper.domain_to_orm(user))  # Обновляем
            else:
                # Если пользователя нет в базе, добавляем нового
                logger.info(
                    f"Пользователь {user.id} не найден. Добавляем нового пользователя."
                )
                self.session.add(
                    UserMapper.domain_to_orm(user)
