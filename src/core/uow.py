    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            if exc_type is not None:
                ...
                logger.warning(
                    f"Исключение в Unit of Work: {exc_type.__name__}: {exc_val}"
                )
    ...
    def commit(self):
        ...
        if self._session:
            try:
                ...
                logger.info(
                    "✅ Unit of Work: транзакция зафиксирована, commit выполнен"
                )
                ...
                logger.error(
                    f"❌ Unit of Work: Транзакция не зафиксирована, commit не выполнен. Ошибка: {e}"
                )
    ...
    def rollback(self):
        ...
        if self._session:
            try:
                ...
                logger.error(
                    f"Unit of Work: КРИТИЧЕСКАЯ ОШИБКА - не удалось выполнить rollback. Ошибка: {e}"
                )
            ...
            logger.warning(
                "Попытка rollback без активной сессии БД - нет транзакции для отката"
            )
