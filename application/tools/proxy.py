...
class ProxyController:
    ...
    def get_proxy(self) -> Optional[str]:
        try:
            ...
            raise ProxyNotFoundError(
                message=f'Неверный формат файла прокси {e}', user_message='❌ '
            )

        except KeyError as e:
            logger.error(f'Отсутствует обязательное поле в прокси: {e}')
            raise ProxyNotFoundError(
                message=f'Неверная структура данных прокси: {e}', user_message='❌ '
            )
    ...
    def check_proxy(self, proxy: str) -> bool:
        ...
        if response.status_code == 200:
            logger.info(f'Прокси успешно проверены, StatusCode {response.status_code}')
            return True
        else:
            logger.info(f'Прокси нерабочие, StatusCode {response.status_code}')
            return False
