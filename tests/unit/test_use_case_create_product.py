...
def test_create_product_success_new_user(
   ...
   # Проверяем вызовы парсера
    ...
    # user сохраняется 2 раза (создание + обновление списка продуктов после append)
    assert pure_mock_user_repo.save.call_count == 2
    # product сохраняется 1 раз (с price_id уже установленным при создании)
    assert pure_mock_product_repo.save.call_count == 1  # ⚡ Изменено с ==2 на ==1
...
def test_create_product_fails_product_exists(
    ...
    pure_mock_parser.parse_product.return_value = {  # Setup
        "id": "p1",
        "name": "Test Product",
    ...
    pure_mock_product_repo.get.return_value = product  # Exists
    ...
    # user_repo.save может быть вызван, если new, но в raise до append/save — проверь в фикстуре
