...
def assert_users_equal(orm_user, domain_user):
    ...
    ), f"Несоответствие ID: {orm_user.id} != {domain_user.id}"
    assert (
        orm_user.username == domain_user.username
    ), f"Несоответствие имени пользователя: {orm_user.username} != {domain_user.username}"
    assert (
        orm_user.chat_id == domain_user.chat_id
    ), f"Несоответствие chat_id: {orm_user.chat_id} != {domain_user.chat_id}"
