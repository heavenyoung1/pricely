...
def test_save_price_success(price_created_first, mock_session):
    ...
    assert (
        orm_obj.id == price_created_first.id
...
@pytest.mark.unit
def test_get_all_prices_by_product(
    mock_session, price_after_checking, orm_price_created_checking
):
    repo = PriceRepositoryImpl(session=mock_session)

    # подсовываем ORM объект как результат запроса
    mock_session.query.return_value.filter_by.return_value.all.return_value = [
        orm_price_created_checking
    ]
